  ...
  return (
    <div className="min-h-screen bg-[#fdfbf9] text-[#171717] flex flex-col">
      ...
      <div className="mx-auto max-w-[1200px] w-full flex-1 grid lg:grid-cols-[1.7fr_0.9fr] gap-0 lg:gap-6 px-0 sm:px-6 py-0 sm:py-6">
        {/* CHAT */}
        <div className="flex flex-col min-h-[calc(100dvh-56px)] sm:min-h-[640px] sm:rounded-[12px] border-0 sm:border border-[#171717] bg-[#fdfbf9] sm:bg-white overflow-hidden shadow-none sm:shadow-[0_8px_24px_rgba(0,0,0,0.06)]">
            ...
            {msgs.map((m,i)=>(
              <div key={i} className={`flex ${m.role==="user"?"justify-end":"justify-start"}`}>
                <div className={`max-w-[86%] rounded-[12px] border px-4 py-3 ${m.role==="user" ? "bg-[#171717] text-white border-[#171717]" : m.is_emergency ? "bg-[#ff6f1e] text-white border-[#ce500a]" : "bg-[#f7efe9] border-[#171717]"} shadow-[0_2px_10px_rgba(0,0,0,0.04)]`}>
                  <div className="text-[14px] leading-6 whitespace-pre-wrap" style={{fontFamily: m.role==="user"?"var(--font-geist)":"var(--font-geist)"}}>{m.content}</div>
                  ...
                  {m.latency!==undefined && <div className="mt-1.5 text-[10px] opacity-60 tracking-wide uppercase">{m.latency}s • grounded • cited</div>}
                </div>
              </div>
            ))}
            {loading && <div className="text-[13px] opacity-60 animate-pulse">thinking from your pdfs… reranking…</div>}
          </div>

          <div className="p-3 sm:p-4 border-t border-[#171717]/10 bg-[#fdfbf9] flex gap-2">
            <input value={q} onChange={e=>setQ(e.target.value)} onKeyDown={e=> e.key==="Enter" && ask()} placeholder="ask — e.g. what are dengue symptoms?" className="flex-1 px-4 py-3 rounded-[20px] border border-[#171717] bg-white text-[14px] outline-none placeholder:opacity-50 focus:ring-2 focus:ring-[#ff6f1e]/20" />
          ...
          <div className="px-4 pb-3 text-[11px] opacity-50">not medical advice — answers only from your PDFs. emergency? call 108/112.</div>
        </div>
